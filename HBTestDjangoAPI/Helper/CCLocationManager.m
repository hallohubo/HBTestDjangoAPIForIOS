...
@property (nonatomic, strong) LocationFinishBlock finishBlock;
...
- (void)getLocationFinished:(LocationFinishBlock)finish{
    self.finishBlock = [finish copy];
    [self startLocation];
}
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    [geocoder reverseGeocodeLocation:newLocation completionHandler:^(NSArray *placemarks, NSError *error){
        ...
        if (placemarks.count > 0) {
            ...
            self->_lastCity = [NSString stringWithFormat:@"%@", placemark.locality];//,placemark.administrativeArea
            [standard setObject:self->_lastCity forKey:CCLastCity];//省市地址
#define s(p, m) (p? p: m)
#define str(p) (p? p: @"")
            self->_lastAddress = [NSString stringWithFormat:@"%@%@%@%@%@%@%@", str(placemark.country), str(placemark.administrativeArea), str(placemark.locality), str(placemark.subLocality), str(placemark.thoroughfare), str(placemark.subThoroughfare), str(placemark.name)];//详细地址
        ...
        if (self->_addressBlock) {
            self->_addressBlock(s(addressDesc, self->_lastAddress));
    ...
    if (self->_finishBlock) {
        self->_finishBlock(self->_lastCity, _lastCoordinate);
    }
...
-(void)startLocation{
    if([CLLocationManager locationServicesEnabled] && [CLLocationManager authorizationStatus] != kCLAuthorizationStatusDenied){
        ...
        Dlog(@"定位服务没开启, 请到设置->隐私, 打开软件的定位服务");
    }
}
