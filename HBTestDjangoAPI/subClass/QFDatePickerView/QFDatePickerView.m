...
//    [yearArray addObject:@"至今"];
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        if (i == 0) {
            ...
            [button setTitleColor:[UIColor colorWithRed:36 / 255.0 green:139 / 255.0 blue:254 / 255.0 alpha:1] forState:UIControlStateNormal];
        }
...
- (void)buttonTapped:(UIButton *)sender {
    if (sender.tag == 10) {
        ...
        if ([selectecMonth isEqualToString:@""]) {//至今的情况下 不需要中间-
            ...
            if(selectecMonth.length >= 2)
            {
                selectecMonth = [selectecMonth substringToIndex:selectecMonth.length - 1];
            }
            
            restr = [NSString stringWithFormat:@"%@-%02ld",selectedYear,selectecMonth.integerValue];
        }
...
//        } else {//非至今的情况下,显示月份
            ...
            NSString * selecYearStr = [selectedYear substringWithRange:NSMakeRange(0, selectedYear.length - 1)];
            
            monthArray = [[NSMutableArray alloc]init];
            if(selecYearStr.integerValue == currentYear)//当前年份
            {
                for (NSInteger i = 1 ; i <= currentMonth; i++) {
                    NSString *monthStr = [NSString stringWithFormat:@"%ld月",(long)i];
                    [monthArray addObject:monthStr];
...
- (NSArray *)monthArr
{
    if(_monthArr == nil)
    {
        ...
        _monthArr = [NSArray arrayWithArray:monthArrM];
    }
    return _monthArr;
}
@end
