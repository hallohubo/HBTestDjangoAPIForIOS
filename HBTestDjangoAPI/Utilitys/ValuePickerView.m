...
#define HEIGHT_OF_POPBOX (([UIScreen mainScreen].bounds.size.width == 414)?320:300)
...
#define ControllerToolBarHeight 44
...
- (void)initSubViews
{
    ...
    self.controllerToolBar.backgroundColor = NJGrayColor(238);
...
- (void)layoutSelfSubviews
{
    ...
    //选择器
    self.statePicker.frame = CGRectMake(0, 40, SCREEN_WIDTH, self.pickerHeight - 20);
    
    //控制栏
    self.controllerToolBar.frame = CGRectMake(0, 0, SCREEN_WIDTH, ControllerToolBarHeight);
    
    //完成按钮
    CGFloat finishBtnHeight = ControllerToolBarHeight;
    self.finishBtn.frame = CGRectMake(SCREEN_WIDTH - 54, 0, 49, finishBtnHeight);
